...
export function createMemoramaGame(parentId, onFinish, onExit) {
  const config = {
    ...
    scene: {
      create() {
        ...
        // ------- Botones superiores -------
        const makeTopBtn = (label, onClick) => {
          const btn = this.add
            .text(0, 18, label, {
              fontFamily: "Arial",
              fontSize: "16px",
        ...
        const doExit = () => {
          stopVoice(); // ✅ corta voz al salir
          if (typeof onExit === "function") onExit();
        };

        const doFinish = (payload) => {
          stopVoice(); // ✅ corta voz al terminar
          if (typeof onFinish === "function") onFinish(payload);
        };

        const ttsBtn = makeTopBtn("Voz: OFF", () => {
          a11y.ttsEnabled = !a11y.ttsEnabled;
          localStorage.setItem("memorama_tts", a11y.ttsEnabled ? "1" : "0");
          updateButtonsText();
          if (a11y.ttsEnabled) say("Voz activada");
          else stopVoice();
        });

        const contrastBtn = makeTopBtn("Contraste: OFF", () => {
        ...
        const exitBtn = makeTopBtn("Salir", doExit);

        const updateButtonsText = () => {
          ttsBtn.setText(a11y.ttsEnabled ? "Voz: ON" : "Voz: OFF");
          contrastBtn.setText(a11y.contrast ? "Contraste: ON" : "Contraste: OFF");
        };
        ...
        const applyContrast = () => {
          ...
          [ttsBtn, contrastBtn, exitBtn].forEach((b) =>
        ...
        const onWin = () => {
          ...
          say(
            `Ganaste. Tiempo ${Math.floor(durationMs / 1000)} segundos. Intentos ${state.attempts}`
          );
  ...
  return () => {
    // ✅ seguridad extra: corta voz al destruir
    try {
      window.speechSynthesis.cancel();
    } catch (_) {}
    game.destroy(true);
